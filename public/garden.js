...
import { copyGarden, rememberGarden, undoGarden } from "./garden-history.js";

export function setupGarden(root, storage = () => globalThis.localStorage) {
  ...
  root.querySelector("#seed-tray").innerHTML = Object.entries(seeds).map(([key, seed]) => `<button type="button" class="seed" data-tool="${key}" aria-pressed="false">${plantArt(key)}<span><strong>${seed.name}</strong><small>${seed.note}</small></span></button>`).join("");

  function chooseTool(tool) {
    activeTool = tool;
    selectedPlot = null;
    root.querySelector("#compact-tool").value = tool;
  ...
  function save() {
    try { storage().setItem(storageKey, JSON.stringify(plots)); canSave = true; }
    catch { canSave = false; }
  ...
  function recordChange(before) {
    history = rememberGarden(history, before, plots);
    save();
  }
  ...
  function render(focusIndex) {
    ...
    root.querySelector("#bloom-count").textContent = `${plots.filter((plant) => plant?.stage === 2).length} of 9 plots in bloom`;
  ...
  grid.addEventListener("click", (event) => {
    ...
    // Copy before watering: growth mutates the existing plant record.
    const before = copyGarden(plots);
    if (activeTool === "move") {
      ...
      if (["moved", "swapped"].includes(move.outcome)) recordChange(before);
      render(index);
  ...
  root.querySelectorAll("[data-tool]").forEach((button) => button.addEventListener("click", () => chooseTool(button.dataset.tool)));
  root.querySelector("#compact-tool").addEventListener("change", (event) => chooseTool(event.target.value));
  ...
  undoButton.addEventListener("click", () => {
    const previous = undoGarden(plots, history);
    if (!previous.changed) return;
    plots = previous.plots;
    history = previous.history;
    selectedPlot = null;
    save(); render();
    status.textContent = "Last change undone. Your plants and their growth are restored.";
    // Keep keyboard focus usable when Undo becomes disabled.
    if (undoButton.disabled) grid.querySelector('[data-plot="0"]').focus({ preventScroll: true });
  });

  root.querySelector("#surprise").addEventListener("click", () => {
    selectedPlot = null;
    render();
    ...
    recordChange(before); chooseTool("water");
  ...
  resetDialog.addEventListener("close", () => {
    ...
    plots = Array(9).fill(null); history = []; save(); chooseTool("daisy");
    status.textContent = "A fresh patch. What will you plant?";
  });
  chooseTool("daisy");
}

if (typeof document !== "undefined") setupGarden(document);
